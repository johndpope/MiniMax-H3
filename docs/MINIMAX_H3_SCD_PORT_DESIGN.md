...
That is a genuine strengthening: a 56% larger video-token budget moving nothing suggests the split is a property of the block stack rather than of the sequence it is fed. It is still one geometry pair, one σ pair, and one temporal length — `latent_t` itself remains unsampled, and testing it needs new prompts, since these describe only the first 29 pixel frames.
