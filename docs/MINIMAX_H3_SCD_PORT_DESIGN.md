...
**There is no real audio anywhere in these numbers, and that is worse than "synthetic content" (2026-08-11).** The caveat above says the content is synthetic. Checking what `build_stream` actually feeds the packer shows the audio half is worse than that: video gets `(1-σ)·latent + σ·ε`, but audio gets **`σ_a·ε` with no latent term at all**, in every run this repo has produced — the clip-sourced runs included. `encode_clip.py` writes video latents only; there is no audio-VAE path in `scripts/scd/`, and the audio VAE weights are not downloaded. So the plan of "re-encode a real clip at 768p to close the caveat" was void before it started: it would have improved the video half of a score that was never in question and left `ccos_audio` measured on Gaussian noise exactly as before.

Real 40 Hz audio latents are strongly autocorrelated and IID noise is the far end of that axis, so `ccos_audio` is an upper bound on the window's cost. The question is whether it is a tight bound or a vacuous one, and that is answerable without the audio VAE: sweep the correlation. `--audio-corr` applies `a[f] = c·a[f-1] + √(1-c²)·ε[f]` along the audio time axis, whose stationary marginal is exactly N(0,1) — so σ_a still scales a correctly-distributed sample and *only* the autocorrelation moves. (The `c·prev + (1-c)·ε` form used for the synthetic video shrinks the variance instead, which would put the rows off-distribution and confound the comparison.) Measured lag-1 cosine tracks `c` to three decimals. At `latent_t`=22, σ=0.571, 768p:

| window | c=0.00 (every prior run) | c=0.70 | c=0.95 | video, all three |
|---|---|---|---|---|
| 8 | 0.714 | 0.679 | 0.760 | 0.9966 |
| 12 | 0.779 | 0.769 | 0.812 | 0.9990 |
| 16 | 0.912 | 0.882 | 0.910 | 0.9999 |

**It does not bind.** Making audio twenty times more autocorrelated moves pooled `ccos_audio` by at most 0.08, non-monotonically, on 284 rows — the size of the run-to-run scatter, not a trend. And the per-frame curve still halves across the post-window span at every `c` (0.851→0.425, 0.700→0.249, 0.737→0.280 at window=12). Video is unchanged to four decimals in all three, which is the sanity check that the knob is reaching audio and nothing else.

That is a useful negative result twice over. It rules out the cheapest explanation of the audio drift — "it is an artefact of feeding audio white noise" — and it independently corroborates the `keep_audio` diagnosis: if the drift were driven by audio's own evicted history, its magnitude would depend on how predictable that history is, and it does not. Both experiments point at the same mechanism, evicted **video** reaching audio through the AV clock. What remains open is narrower than before: correlation is measured and does not matter, but correlated noise still carries no *signal*, so a run with genuine audio latents would test something these do not.

**So it stays opt-in, and the open question moves to Phase 3.** Three reasons not to widen the window further and call it solved: audio carries no signal in any of these runs (its *correlation* is now measured and does not bind, but its content is still noise); σ=0.923 is consistently kinder than σ=0.571, so the number depends on where in the schedule it is read; and above all **these weights are untrained for this split**. Phase 0 measured how much causal masking an unadapted model tolerates, and SCD's entire premise is that training closes that gap. A drift measured on an unadapted encoder is the input to Phase 3, not a verdict on the architecture. What this does settle is the ranking: `window=12, keep_audio=True` is the configuration to train against, audio quality is the metric to watch while doing it, and video is not the thing at risk.
...
**Speedup, at the one length where both sides run: 2.93× (N=8), 3.51× (N=16), 3.87× (N=30)** — on a decoder packed as `2R` rows, which is an assumption Phase 3 may not keep; see *The decoder timed here is 2R rows* below, where adding §6.2's text rows takes N=30 to 3.18×. The gate was "materially faster"; Tier 0 predicted 3.90× at 768p/10s on a cost model that had the split wrong in three places, and Tier 1 lands on the same order from real weights. The win grows with N for the reason Tier 0 gave — the encoder is paid once — and its ceiling is `stock_step / (latent_t × decoder_frame)` = 11.219 / (12 × 0.2129) = **4.39×**, which is what N→∞ buys and what a later step-count increase would asymptote to.
...
**The decoder timed here is 2R rows, and §6.2's decoder is not (2026-08-11) — `docs/phase25_tier1_dectext.json`.** `time_decoder_frame` packs the conditioning half and the noisy half and nothing else. The Phase 3 pseudocode above packs *"noisy video frame tokens (+ text; audio policy per D1)"*. Text is 512 rows against 2R = 2016 at 768p, so the two differ by a 25% longer sequence, and this term multiplies `n_steps × frames` — it is the one place where a wrong assumption scales into the headline. Measured side by side in one run at `latent_t`=12, `window=8`:

| decoder pack | ms/frame | peak | N=8 | N=16 | N=30 |
|---|---|---|---|---|---|
| `2R` (what Tier 1 quotes) | 213.0 | 3.7 GB | 2.96× | 3.54× | 3.90× |
| `2R + text` (what §6.2 specifies) | **267.7** | 3.7 GB | **2.52×** | **2.94×** | **3.18×** |
...
#### Data loader and adapters shipped, and what the clip set can and cannot answer (2026-08-11)

`scripts/scd/scd_data.py` and `scripts/scd/scd_lora.py`, the two pieces between `decode_frame` and a train loop.

**The clip set is 12 clips, not the 20–50 the phase asks for**, and three of its properties bound what a run on it means. It is 512×512 → 32×32 latents → **256 rows per latent frame against 768p's 1008**, so nothing trained here validates the 768p geometry; it validates the mechanism at a quarter of the rows, which is the same gap §11 already flags as the reason not to benchmark speed at training scale. It has **no audio** — the clips carry video latents only and the base draws its own noise rows — so the audio-side window drift measured in Phase 2 is not something a run here can improve or refute. And `isodiorama640` is the same source video and a byte-identical prompt as `isodiorama`, encoded at 640² as Phase 0's geometry control; as training data that is a near-duplicate at a different `frame_rows`, so it is **held out by default** and `include_control=True` puts it back.

Batch size is 1 and stays 1. Text length varies 226–265 tokens across the set, so a batch would need padding in the **text** segment, and `text_len` is baked into the packer's RoPE positions and modulation rows — padding it is a change to the one thing `preamble` exists to avoid re-deriving. Gradient accumulation is the lever, not batching. MANIFEST.tsv rather than a glob is the source of truth for the same reason it was written: the `.safetensors` are gitignored, and a glob would train on whatever geometry was left over from a re-encode.

**Adapters are wrappers, not forward-patches, and they carry D3's asymmetry as an installed fact.** The encoder excludes `adaln_proj` (it runs at a single σ=0, so that projection's gradient is rank-1 and the capacity buys a scaled constant); the decoder **keeps** it, because `decoder_frame_input` puts its two halves at different timesteps by construction, so decoder modulation is the one place the split changes what that layer is asked to do. Nothing touches `base.weight` — no merge path, no `weight.data` arithmetic — which is what makes the adapter indifferent to whether the target is `nn.Linear`, bnb's `Linear4bit` or Fizgig's `ConvRotInt8Linear`.

Two things that only showed up by testing them:

- **`scd.to(torch.bfloat16)` silently undid the fp32 adapters.** It recurses, so the one line every caller writes to get the base into bf16 took the factors with it — at which point an update of ~1e-4 against a weight of ~1e-2 lands below bf16's ulp and is dropped, which presents as a LoRA that will not learn. `LoRALinear._apply` now puts them back after any dtype cast while letting device moves through.
- **Decoder adapters cannot use base block names.** The re-composition holds blocks 0 and 1 **twice**, once per half, so naming both `lora_unet_blocks_0_…` would have one overwrite the other on save and apply the decoder's adaptation to the encoder's block. They get `lora_unet_scddec_<source>_…`, keyed on the **source** block rather than the slot — slot and source agree on the front pair and diverge on the tail, so a slot-named checkpoint passes any test that only checks the shared blocks while claiming to adapt blocks the composition never touched. Encoder names stay drop-in for the stock graph.
...
| **Phase 2 window cost** | ⚠️ **SPLIT by modality, confirmed at 768p.** Video's windowed error is **bounded** — per-frame ccos asymptotes at 0.989 and flattens. Audio's **accumulates** — 0.83 → 0.39 across the span past the window, at every width tried. `keep_audio=True` (window video only, +1% cache) lifts pooled audio 0.779 → 0.890 at window=12 but does not flatten the curve, because the drift enters through evicted *video* that audio attends to. Sweeping audio's autocorrelation (`--audio-corr` 0 → 0.95) moves the number by ≤0.08 non-monotonically and does not flatten it either, which rules out "artefact of white-noise audio" and corroborates the same mechanism. Measured on weights untrained for this split, and with audio carrying no signal in any run, so it is Phase 3's input, not a verdict |
| **Phase 2.5 Tier 0** | ✅ **PASSED** — 3.90× at 768p/10s, 5.03× at 15s (gate was ≥2×) |
| **Phase 2.5 Tier 1** | ✅ **PASSED (2026-08-11)** — 2.93×/3.51×/3.87× at N=8/16/30 on real NF4 weights at 768p, `latent_t`=12, ceiling 4.39×. Decoder flat at ~214 ms/frame and cache flat at 12758→12796 rows across a 2.6× length span. The result the gate did not ask for: **stock OOMs past ~2 s at 768p on 24 GB** and SCD does not, so at 15 s there is no ratio to take. Two caveats: 15 s itself is unmeasured on this card for stock, so the gate is met by extrapolation plus a hard stock ceiling rather than a 15 s A/B; and the decoder is timed at `2R` rows, so if Phase 3 packs text as §6.2 specifies the N=30 figure is **3.18×, not 3.87×** |
